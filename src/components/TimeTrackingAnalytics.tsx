            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={({ taskTitle, hours }) => `${taskTitle.substring(0, 15)}: ${hours}h`}
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="hours"
                >
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={analytics.entriesByWeek}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="week" tickFormatter={(week) => new Date(week).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} />
                <YAxis />
                <Tooltip />
